...
## 操作步骤 {#section_7r4_7ks_l4b .section}

1.  初始化驱动资源。调用leda\_init接口完成资源初始化。

    ``` {#codeblock_g4r_exr_zo4}
...
2.  解析驱动配置，完成设备上线。通过leda\_get\_device\_info\_size接口和leda\_get\_device\_info接口获取设备配置，按照驱动配置格式解析，根据解析后的驱动配置连接设备并调用leda\_register\_and\_online\_by\_device\_name接口完成设备注册上线到[阿里云物联网平台](https://iot.console.aliyun.com/)。

    驱动信息配置格式定义请参见本文上方[驱动与设备信息配置](#section_75g_3ng_a20)内容。

    ``` {#codeblock_vf0_n1d_7qv}
